...
class SlackvarkBot:
    ...
    def getDirectChannelID(self, username):
        ...
        imObjects = self.client.api_call(method="im.list").decode("utf-8") # list all direct message channels
    ...
    def createNewChannel(self, users, groupName):
        ...
        groupObject = self.client.api_call(method = "groups.create", token=human_token, name=groupName) # create group
        ...
        for userName in users:
            ...
            self.client.api_call(method = "groups.invite", token=human_token, channel=groupId, user=self.getUserID(userName)) # invite to group

        self.client.api_call(method = "groups.invite", token=human_token, channel=groupId, user=self.getUserID("slackvark_bot")) ## invite self
    ...
    def processMessage(self, action):
        ...
        if command == "hello": # bot listens for "hello"
            ...
            self.post(self.getDirectChannelID(action["user"]), "hello") # messages "hello" via DM
        elif command == "3":
            self.post(action["channel"], "Awesome. Who would you like to invite?") # prompts for list of users
            condition = True
            while condition: # wait for response from user
                ## begin code repeated from listener. This may be a good candidate for a function. ##
                response1 = self.client.rtm_read()
                for item in response1:
                    if "type" in item and item["type"] == "message":
                        usernameList = item["text"].strip().split(" ") ## split on spaces
                        condition = False
                ## --------------------------- end repeated code --------------------------------- ##
